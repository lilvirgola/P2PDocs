defmodule P2PDocs.Network.NeighborHandler do
  ...
  defmodule State do
    ...
    """
    # enforce that socket and peers are always present
    @enforce_keys [:socket, :peers]
    defstruct [
      :socket,
      :node_name,
      peers: %{},
      subscribers: %{},
      # Default value directly here
      port: 45892,
      # Default value directly here
  ...
  def handle_info(:send_heartbeat, state) do
    ...
    {active_peers, expired_peers} =
      Enum.split_with(state.peers, fn {_, last_seen} ->
        now - last_seen < 3 * div(state.interval, 1000)
      end)

    # Notify about expired peers
    expired_peers
    # map the expired items to a list of maps
    |> Enum.map(fn {{name, ip, port}, _} -> %{name: name, ip: :inet.ntoa(ip), port: port} end)
    |> Enum.each(fn peer ->
      # notify the subscribers about the expired peers
      notify_subscribers(state.subscribers, {:peer_expired, peer})
    end)
  ...
  def handle_info({:udp, _socket, ip, port, packet}, state) do
    case decode_heartbeat(packet, state.secret) do
      # If the packet is valid, we decode it and get the peer name
      {:ok, peer_name} ->
        if peer_name != state.node_name do
          ...
          if is_new_peer do
            ...
            # try to convert the peer name to an existing atom, if doesn't exists, catch error and create a new atom
            try do
      ...
      # If the packet is invalid, we log a warning
      :error ->
