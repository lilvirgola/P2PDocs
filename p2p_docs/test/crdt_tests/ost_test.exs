...
defmodule OSTree.CorrectnessTest do
    ...
    test "inserting values yields sorted order via kth_element/2" do
      values = [5, 1, 3, 2, 4]
      tree = Enum.reduce(values, OSTree.new(fn a, b -> a - b end), fn x, acc -> OSTree.insert(acc, x) end)
    ...
    test "custom comparator for descending order" do
      comp = fn a, b -> b - a end
      values = [10, 20, 15]
      tree = Enum.reduce(values, OSTree.new(fn a, b -> b - a end), fn x, acc -> OSTree.insert(acc, x) end)
  ...
  defmodule OSTree.PerformanceTest do
    ...
    test "insert 10_000 elements efficiently" do
      {time, tree} = :timer.tc(fn ->
        Enum.reduce(1..10_000, OSTree.new(fn a, b -> a - b end), fn x, acc -> OSTree.insert(acc, x) end)
      end)
    ...
    test "kth_element selection for 10_000 elements" do
      tree = Enum.reduce(1..10_000, OSTree.new(fn a, b -> a - b end), fn x, acc -> OSTree.insert(acc, x) end)
      {time, _} = :timer.tc(fn ->
        for k <- 1..10_000 do OSTree.kth_element(tree, k) end
      end)
    ...
    test "delete 5_000 random elements efficiently" do
      ...
      tree = Enum.reduce(initial, OSTree.new(fn a, b -> a - b end), fn x, acc -> OSTree.insert(acc, x) end)
      to_delete = Enum.take_random(initial, 5_000)
  
      {time, tree2} = :timer.tc(fn ->
