defmodule CustomBroadcast do
  def sendMessage(_op, _payload) do :ok end
end

defmodule Math do
  def ilog(n) when n < 2 do 0 end
  def ilog(n) do 1 + ilog(div(n,2)) end
end

defmodule CrdtTextTest do
  use ExUnit.Case
  alias CrdtText
  ...
  test "random local inserts maintain ordering and uniqueness", %{state: state} do
    ...
    final_state =
      Enum.reduce(1..total, state, fn i, st ->
        ...
        CrdtText.local_insert(st, idx, Integer.to_string(i))
      end)
